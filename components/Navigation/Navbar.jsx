...
export const Navbar = ({ onMenuClick }) => {
    ...
    const logoColor = pathname === "/agence" || pathname === "/projects" ? "black" : "white";
    ...
    return (
        <div className="z-4 flex fixed top-0 w-full items-start justify-between">
            ...
            <div onMouseEnter={() => setIsHovered(true)}
                ...
                onClick={onMenuClick}
                className="bg-black lg:h-13 h-11 relative lg:w-60 w-52 cursor-pointer flex items-center justify-end overflow-hidden">
                <div
                    className={`bg-[#d3fd50] transition-all absolute top-0 left-0 w-full ${isHovered ? 'h-full' : 'h-0'}`}
                ></div>
                <div className="relative z-10 flex flex-col items-end gap-y-1 lg:pr-8 pr-6">
                    <div className={`h-[1.5px] lg:w-13 w-12 transition-colors ${isHovered ? 'bg-black' : 'bg-white'}`}></div>
                    <div className={`h-[1px] w-6 transition-colors ${isHovered ? 'bg-black' : 'bg-white'}`}></div>
                </div>
            </div>
        </div>
    )
